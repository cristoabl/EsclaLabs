  ...
  useEffect(() => {
    ...
    const render = () => {
      if (video && !isNaN(targetTimeRef.current)) {
        ...
        if (!isNaN(currentTimeRef.current)) {
    ...
    <section ref={containerRef} className="relative h-[400vh] bg-black">
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden z-10 px-6">
        ...
        <motion.div style={{ opacity: opacity1, y: y1 }} className="absolute text-center max-w-5xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-cyan-500/20 bg-cyan-500/5 mb-8 shadow-inner">
            <Zap className="w-3 h-3 text-cyan-500" />
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-400">EsclaLabs v2.1.2</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-24 flex flex-col md:flex-row justify-between items-end gap-10">
           <div className="text-left">
              ...
              <h2 className="text-6xl md:text-8xl font-black leading-[0.8] tracking-tightest uppercase italic text-white">Soluciones <br /> <span className="text-white/10 text-left block">Modulares.</span></h2>
           </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            { title: "Web3 Hub", desc: "Integración nativa con protocolos DeFi y tesorería on-chain.", icon: <Globe className="w-8 h-8" />, img: "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&q=80&w=800" }
          ].map((s, i) => (
            <div key={i} className="group relative h-[500px] rounded-[3rem] overflow-hidden border border-white/5 bg-neutral-900 transition-all hover:border-cyan-500/30 text-left">
      ...
      <main>
        ...
        <div className="relative z-20 py-12 border-y border-white/5 bg-black overflow-hidden">
          <div className="flex whitespace-nowrap animate-infinite-scroll">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="flex items-center gap-20 mx-10 font-black text-2xl uppercase italic tracking-widest text-white/5">
                <span>Maña</span>
      ...
      <footer className="relative z-20 py-20 px-6 border-t border-white/5 text-center bg-black">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/30 text-xs max-w-md mx-auto mb-10 leading-relaxed italic">
            Arquitectando la frontera de las finanzas digitales y la inteligencia autónoma. v2.1.2
          </p>
